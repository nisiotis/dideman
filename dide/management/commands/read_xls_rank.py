    ...
    def handle(self, *args, **options):
        for item in args:
            ...
            while curr_row < worksheet.nrows:
                ...
                firstname = str(worksheet.cell_value(curr_row, 1))
                lastname = str(worksheet.cell_value(curr_row, 0))
                fathername = str(worksheet.cell_value(curr_row, 2))
                
                rank = str(worksheet.cell_value(curr_row, 4))
                ...
                if emps:
                    if len(emps) == 1:
                        strsql = "insert into dide_promotionnew (Id, employee_id, rank, value_id, date, time_left, `order`,order_pysde) values (NULL, %s, '%s', %s, '2016-01-01', '%s', '%s', '');" % (emps[0].parent_id, str(rank), str(emps[0].ranknew()).split(" ")[1], time_left, 'από μεταφορά')
